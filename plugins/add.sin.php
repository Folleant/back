...
if ($_POST['rid'] == $lang_grabber['lang_add'] and !empty($_POST['new']) and !empty($_POST['var'])) {
    $newsin = $db->safesql($_POST['new'] . '|' . $_POST['var']);
...
} elseif ($_POST['rid'] == $lang_grabber['lang_add'] and ($_POST['new'] == '' or $_POST['var'] == '')) {
    msg($lang_grabber['dod_add'], '<b>' . $lang_grabber['add_sin_database'] . '</b>', '<font color=red>' . $lang_grabber['no_pol_zap'] . '</b>', $PHP_SELF . '?mod=rss&action=sinonim');
}
...
<input type=\"submit\" style=\"width: 100px;\" class=\"btn btn-success\" name=\"rids\" value=\"{$lang_grabber['lang_rided']}\">
    ...
    <input type=\"submit\" style=\"width: 100px;\" class=\"btn btn-warning\" name=\"rid\"  value=\"{$lang_grabber['lang_sbros']}\"></center>
        ...
        msg($lang_grabber['red_grups'], '<b>' . $lang_grabber['red_sins'] . '</b>', '<font color=red>' . $lang_grabber['yj_red_sins'] . '</font>', $PHP_SELF . '?mod=rss&action=sinonim');
    }
}
...
if ($_POST['rids'] == $lang_grabber['lang_rided']) {
    ...
    for ($x = 1; $x <= $_POST['kol']; $x++) {
        if ($_POST['new' . $x] != $_POST['newo' . $x] or $_POST['var' . $x] != $_POST['varo' . $x]) {
            $newsin = $db->safesql($_POST['new' . $x] . '|' . $_POST['var' . $x]);
            if ($_POST['id' . $x] != '' and $_POST['new' . $x] != '' and $_POST['var' . $x] != '') {
                ...
                $db->query("DELETE  FROM " . PREFIX . "_synonims WHERE id = '" . $_POST['id' . $x] . "' ");
                
                $mes_sin .= '<font color=red><b>' . $_POST['new' . $x] . '</b><b>' . $_POST['var' . $x] . '</font> - <font color=red>удален</font></b><br />';
                
            }
        }
    }
    if ($mes_sin != '') {
        $mes_sin = '<font color=red>' . $lang_grabber['red_sin_ok'] . '</font><br /><br />' . $mes_sin;
        msg($lang_grabber['red_grups'], '<b>' . $lang_grabber['red_sins'] . '</b>', $mes_sin, $PHP_SELF . '?mod=rss&action=sinonim');
    } else {
        msg($lang_grabber['red_grups'], '<b>' . $lang_grabber['red_sins'] . '</b>', $lang_grabber['red_sin_on'], $PHP_SELF . '?mod=rss&action=sinonim');
    }
}
...
<input type=\"submit\" style=\"width: 100px;\" class=\"btn btn-success\" name=\"rid\"  value=\"{$lang_grabber['lang_rided']}\">
    ...
    <input type=\"submit\" style=\"width: 100px;\" class=\"btn btn-warning\" name=\"rid\"  value=\"{$lang_grabber['lang_sbros']}\">";
        } else {
            echo "<font color=\"#8C8C8C\"><b>{$lang_grabber['dub_sins']}</b><br>
