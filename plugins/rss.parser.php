...
$story = preg_replace_callback( "#<img(.+?)src[=]?[='\"](\S+?)['\"](.*?)>#i", "decode_img_rss", $story );
	...
	$story = preg_replace( "#<strong>(.+?)</strong>#is", "[b]\\1[/b]", $story );

	$story = preg_replace( "#<blockquote class=[\"']instagram-media[\"'].*?<a href=[\"'](.*?)[\"'].*?<\/blockquote>#is", "[media=\\1]", $story );
	
	$story = preg_replace( "#<blockquote class=[\"']twitter-tweet[\"'].*?<a href=[\"']https\:\/\/twitter.com(\S+)\/status\/(.*?)[\"'].*?<\/blockquote>#is", "[media=https://twitter.com$1/status/$2]", $story );
	
	$story = preg_replace( "#<blockquote>(.+?)</blockquote>#is", "[quote]$1[/quote]", $story );


$story = preg_replace( "#<a.*?href[=]?[='\"](.+?)['\" >].*?>(.*?)<\/a>#is", "[url=\\1]\\2[/url]", $story );

$story = preg_replace( "#<a.*?href[=]?[='\"](.+?)['\" >].*?>#is", "", $story );
$story = preg_replace( "#<\/a>#is", "", $story );
...
{return $string;
	...
	$str = str_replace ('[medium', '[img', $str);
	$str = str_replace ('medium]', 'img]', $str);
	$str = str_replace (']]>', '', $str);
$keys = array(
...
$story = preg_replace ('#\\[media=(.+?)\\]#is', '[center]\\0[/center]', $story);
$story = preg_replace ('#<iframe.+?<\/iframe>#i', '[center]\\0[/center]', $story);
...
if (!(in_array($str,$strs)) and strlen ($s) > 2 )
	{
$strs []= $str;
$sort []= $s;
}
...
function img_detect($text, $match, $num = 0)
{$stop = $match[2];
    if (!empty($match[0])) {
        if ($num == 1) {
            $medium_new  = str_replace($match[1][0], 'thumb', $match[0][0]);
            $match[0][0] = addcslashes(stripslashes($match[0][0]), "[]!-.#?*%+\\:()|");
            $text        = preg_replace("#" . $match[0][0] . "#is", $medium_new, $text, 1);
            $stop        = array( $match[2][0] );
        } elseif ($num == 2) {
            $text       = preg_replace("#\[(medium|img)(.*?)\](.*?)\[\/(medium|img)\]#is", "[thumb$2]$3[/thumb]", $text);
            $medium_new = str_replace($match[1][0], 'thumb', $match[0][0]);
            $medium_new = addcslashes(stripslashes($medium_new), "[]!-.#?*%+\\:()|");
            $text       = preg_replace("#" . $medium_new . "#is", $match[0][0], $text, 1);
            unset($match[2][0]);
            $stop = $match[2];
        } elseif ($num == 3) {
            $text = preg_replace("#\[(medium|img)(.*?)\](.*?)\[\/(medium|img)\]#is", "[thumb$2]$3[/thumb]", $text);
 ...
 return array('story' => $text,'stop' => $stop);}
...
$quotes = array(  "\t",'\n','\r', "\n","\r", '\\',",",".","/","¬","#",";",":","@","~","[","]","{","}","=","-","+",")","(","*","^","%","$","<",">","?","!", '"', ',,','/','//','&raquo;','|',':',' ',',,','(',')','-' );
	...
	$story = str_replace( "&nbsp;", " ", $story );
	$story = str_replace( '<br />', ' ', $story );
	preg_match_all('/([а-яА-ЯA-Za-z0-9]+)/', $story, $words);
...
if (!(in_array ($str,$strs)) and strlen ($key) > 3 )
	{
$strs []= $str;
$stor [$key]= $key;
$sort []= $key;
...
', $story);
return $story;}
function replace_url ($str, $st = false){if(!$st)return $str;
$keys = array ( 'А' => '%D0%90', 'Б' => '%D0%91', 'В' => '%D0%92', 'Г' => '%D0%93', 'Д' => '%D0%94', 'Е' => '%D0%95', 'Ё' => '%D0%81', 'Ж' => '%D0%96', 'З' => '%D0%97', 'И' => '%D0%98', 'Й' => '%D0%99', 'К' => '%D0%9A', 'Л' => '%D0%9B', 'М' => '%D0%9C', 'Н' => '%D0%9D', 'О' => '%D0%9E', 'П' => '%D0%9F', 'Р' => '%D0%A0', 'С' => '%D0%A1', 'Т' => '%D0%A2', 'У' => '%D0%A3', 'Ф' => '%D0%A4', 'Х' => '%D0%A5', 'Ц' => '%D0%A6', 'Ч' => '%D0%A7', 'Ш' => '%D0%A8', 'Щ' => '%D0%A9', 'Ь' => '%D0%AC', 'Ъ' => '%D0%AA', 'Ы' => '%D0%AB', 'Э' => '%D0%AD', 'Ю' => '%D0%AE', 'Я' => '%D0%AF', 'а' => '%D0%B0', 'б' => '%D0%B1', 'в' => '%D0%B2', 'г' => '%D0%B3', 'д' => '%D0%B4', 'е' => '%D0%B5', 'ё' => '%D1%91', 'ж' => '%D0%B6', 'з' => '%D0%B7', 'и' => '%D0%B8', 'й' => '%D0%B9', 'к' => '%D0%BA', 'л' => '%D0%BB', 'м' => '%D0%BC', 'н' => '%D0%BD', 'о' => '%D0%BE', 'п' => '%D0%BF', 'р' => '%D1%80', 'с' => '%D1%81', 'т' => '%D1%82', 'у' => '%D1%83', 'ф' => '%D1%84', 'х' => '%D1%85', 'ц' => '%D1%86', 'ч' => '%D1%87', 'ш' => '%D1%88', 'щ' => '%D1%89', 'ь' => '%D1%8C', 'ъ' => '%D1%8A', 'ы' => '%D1%8B', 'э' => '%D1%8D', 'ю' => '%D1%8E', 'я' => '%D1%8F', ' ' => '%20', '«' => '%C2%AB', '»' => '%C2%BB', '–' => '%E2%80%93');
foreach ($keys as $key => $value){
...
if ($dude_leech != '')$story = preg_replace_callback ('#\[url=(.+?'.addcslashes(stripslashes($leech_shab),'"[]!-.#?*%\\()|/:+').'.+?)\](.+?)\[\/url\]#s', "dude_noleech", $story);
$story = preg_replace_callback ('#\[url=(.+?\/lock\/.+?)\](.+?)\[\/url\]#s', "dude_noleech", $story);
$story = preg_replace_callback ('#\[url=(.+?\/out.+?)](.+?)\[\/url\]#s', "dude_noleech", $story);
$story = preg_replace_callback ('#\[url=(.+?\/engine\/go\.php\?url=.+?)\](.+?)\[\/url\]#s', "dude_noleech", $story);
$story = preg_replace_callback ('#\[url=(.+?dude.+?)\](.+?)\[\/url\]#s', "dude_noleech", $story);
$story = preg_replace_callback ('#\[url=(.+?leech_out.+?)\](.+?)\[\/url\]#s', "dude_noleech", $story);
...
if (($pd = strtotime(add_data (rss_strip ($d)))) === false) {
  $pd=preg_match("#(\S+)[.,\/\- ]+(\S+)[.,\/\- ]+(\S+)#iu",str_replace("&nbsp;", " ",$d),$matches);
//print_r($matches);
  if($pd)
  {
$month[1] = "янв";
...
$month[12] = "дек";
foreach ($month as $key=>$value){
if (preg_match('#'.$value.'#iu',$matches[2]))$matches[2]=$key;
}
...
function escape_win ($path) {
	global $config;
$str = $path;
$escape = array("\u0022"=> '"', "\u0020"=> " ", "\U0430"=>"а", "\U0431"=>"б", "\U0432"=>"в", 
...
$path = preg_replace_callback('!#u([a-zA-Z0-9]{4})!is', 'dec_java',$path);
$path = html_entity_decode($path, ENT_QUOTES|ENT_HTML401, $config['charset']);
if(empty($path))$path = $str;
return $path; 
}

function dec_java ($matches=array()){
	$path = preg_replace("#[0]+(.+)#", '&#x\\1;' ,$matches[1]);
	
	if(empty($path))$path = $matches[1];
	return $path; 
}
