...
case "list":
    ...
    foreach ($xfields as $name => $value) {
      ...
      if ($value[3] == "textarea" or $value[3] == "imagegalery") {
      $output .= <<<HTML
<tr id="$holderid" {$uid}>
<td width="140">$value[1]: [not-optional]<span style="color:red;">*</span>[/not-optional]<br />[if-optional]({$lang['xf_not_notig']})[/if-optional]</td>
...
<td class="xfields"><textarea name="xfield$i$channel_id.[$fieldname]" id="xf_$i$channel_id$fieldname">$fieldvalue[$fieldname]</textarea></td></tr>
HTML;
		}elseif ($value[3] == "select") {
		
			$select = "<select class=\"uniform\" style=\"min-width:140px;\" name=\"xfield$i$channel_id.[$fieldname]\" multiple=\"multiple\">";
        ...
        foreach (explode("\r\n", $value[4]) as $index1 => $value1) {
		  ...
		  $value_id = explode( ",",  $fieldvalue[$fieldname]);
		  	  
          $select .= "<option value=\"$index1\"";

					if( is_array( $value_id ) ) {
						foreach ( $value_id as $element ) {
							if( trim($element) == $value1[1] ) $select .= 'selected';
						}
					} elseif( trim($value_id) == $value1[1] ) $select .= 'selected';

		  $select .= ">{$value1[1]}</option>\r\n";
        }
		$select .= "</select>";
			$output .= <<<HTML
<tr id="$holderid">
<td  width="140">$value[1]: [not-optional]<span style="color:red;">*</span>[/not-optional]</td>
